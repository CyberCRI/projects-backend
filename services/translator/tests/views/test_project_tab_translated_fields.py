...
class ProjectTabTranslatedFieldsTestCase(JwtAPITestCase):
    ...
    def test_create_project_tab(self):
        ...
        self.assertEqual(
            auto_translated_fields.count(), len(ProjectTab.auto_translated_fields)
        )
        self.assertSetEqual(
            {field.field_name for field in auto_translated_fields},
            set(ProjectTab.auto_translated_fields),
        )
    ...
    def test_update_project_tab(self):
        ...
        payload = {
            ProjectTab.auto_translated_fields[0]: faker.word(),
        }
        ...
        payload = {
            ...
            for translated_field in ProjectTab.auto_translated_fields
        }
        response = self.client.patch(
            reverse("ProjectTab-detail", args=(self.project.id, project_tab.pk)),
            data=payload,
