...
# ORGANIZATION_CODE = "DEFAULT"  # noqa
ORGANIZATION_CODE = "CRI"
# GROUP_TO_TEST = "testing_a6grj"  # noqa
...
def prompt_login() -> str:
    ...
    CALLBACK_URL = input("Enter the full callback URL")  # noqa

    token = oauth.fetch_token(
        token_url, authorization_response=CALLBACK_URL, client_secret=client_secret
    )
    b64_token = base64.b64encode(bytes(json.dumps(token), "utf-8")).decode("utf-8")
    print(
        f"Token: {b64_token}. Please set this as an environment variable TOKEN for future runs."
    )
    return token
...
class User(HttpUser):
    ...
    def get_people_group_by_id(self):
        self.client.get(
            f"/v1/organization/{ORGANIZATION_CODE}/people-group/{GROUP_TO_TEST}/"
        )

    @task
    def get_people_group_member(self):
        self.client.get(
            f"/v1/organization/{ORGANIZATION_CODE}/people-group/{GROUP_TO_TEST}/member?limit=30"
        )

    @task
    def get_people_group_project(self):
        self.client.get(
            f"/v1/organization/{ORGANIZATION_CODE}/people-group/{GROUP_TO_TEST}/project/"
        )

    @task
    def get_recommended_project_user(self):
        self.client.get(
            f"/v1/organization/{ORGANIZATION_CODE}/recommended-project/user/"
        )

    @task
    def get_recommended_project_user_random(self):
        self.client.get(
            f"/v1/organization/{ORGANIZATION_CODE}/recommended-project/user/random/"
        )
    ...
    def get_recommended_user_user_random(self):
        self.client.get(
            f"/v1/organization/{ORGANIZATION_CODE}/recommended-user/user/random/"
        )
