...
class RetrieveProjectImageTestCase(JwtAPITestCase):
    @classmethod
    ...
    def test_retrieve_project_image(self, role, retrieved_images):
...
class UpdateProjectImageTestCase(JwtAPITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.project.images.add(cls.image)
    ...
    def test_update_project_image(self, role, expected_code):
        user = self.get_parameterized_test_user(
...
class DeleteProjectImageTestCase(JwtAPITestCase):
    @classmethod
    def setUpTestData(cls):
        super().setUpTestData()
        cls.organization = OrganizationFactory()
    ...
    def test_delete_project_image(self, role, expected_code):
...
class MiscProjectImageTestCase(JwtAPITestCase):
    def test_multiple_lookups(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
        response = self.client.get(
            reverse("Project-images-detail", args=(project.slug, image.id)),
        )
        self.assertEqual(response.status_code, status.HTTP_302_FOUND)
