...
class ProjectRemoveTeamMembersSerializer(serializers.Serializer):
    ...
    def validate_people_groups(
        ...
        return list(
            filter(
                lambda x: x.groups.filter(projects=project).exists(),
                people_groups,
            )
        )
...
class ProjectMessageSerializer(
    ...
    reply_on = serializers.PrimaryKeyRelatedField(
        queryset=ProjectMessage.objects.all(), required=False, allow_null=True
    )
...
class ProjectTabItemSerializer(
    ...
    class Meta:
        ...
        fields = read_only_fields + ["title", "content", "images"]

    def get_string_images_kwargs(
        self,
        instance: ProjectTabItem,
        field_name: str,
        *args: Any,
        **kwargs: Any,
    ) -> dict[str, Any]:
