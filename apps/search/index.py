...
class UserIndex:
    ...
    def prepare_skills(user: ProjectUser) -> List[str]:
        ...
        return [skill.tag.title for skill in user.skills.all()]

    @staticmethod
    def prepare_skills_filter(user: ProjectUser) -> List[str]:
        """Return the skills' qids for Algolia filtering."""
        return [skill.tag.id for skill in user.skills.all()]
    ...
    def prepare_can_mentor_on_filter(user: ProjectUser) -> List[str]:
        """Return the skills' qids for Algolia filtering."""
        return [skill.tag.id for skill in user.skills.filter(can_mentor=True)]

    @staticmethod
    def prepare_needs_mentor_on_filter(user: ProjectUser) -> List[str]:
        """Return the skills' qids for Algolia filtering."""
        return [skill.tag.id for skill in user.skills.filter(needs_mentor=True)]
