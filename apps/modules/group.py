...
class PeopleGroupModules(AbstractModules):
    ...
    def featured_projects(self) -> QuerySet[Project]:
        ...
        return (
            self.outer_ref(self.user.get_project_queryset())
            .filter(
    ...
    def similars(self) -> QuerySet[PeopleGroup]:
        base_qs = self.outer_ref(GroupEmbedding.objects.all())
        qs_similar = base_qs.vector_search(self.instance, 0.15)

        group_qs = self.outer_ref(self.user.get_people_group_queryset())

        return group_qs.filter(embedding__in=qs_similar)

    def subgroups(self) -> QuerySet[PeopleGroup]:
        return PeopleGroup.objects.filter(
            parent=self.instance, pk__in=self.user.get_people_group_queryset()
        )
    ...
    def _documents(self, documents_type: Iterable[str]) -> QuerySet[Document]:
        members_qs = self.members()
        return (
            self.outer_ref(Document.objects)
            .filter(document_type__in=documents_type, contributors__user__in=members_qs)
            .distinct()
        )
