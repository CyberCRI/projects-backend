...
class MiscUserProfilePictureTestCase(JwtAPITestCase):
    def test_multiple_lookups(self):
        user = UserFactory(profile_picture=self.get_test_image())
        self.client.force_authenticate(user)
        payload = {
            "scale_x": faker.pyfloat(min_value=1.0, max_value=2.0),
        }
        response = self.client.patch(
            reverse(
                "UserProfilePicture-detail", args=(user.id, user.profile_picture.id)
            ),
            data=payload,
            format="multipart",
        )
        ...
        content = response.json()
        self.assertEqual(content["id"], user.profile_picture.id)
        response = self.client.patch(
            reverse(
                "UserProfilePicture-detail", args=(user.slug, user.profile_picture.id)
            ),
            data=payload,
            format="multipart",
        )
        ...
        response = self.client.patch(
            reverse(
                "UserProfilePicture-detail",
                args=(user.keycloak_id, user.profile_picture.id),
            ),
            data=payload,
            format="multipart",
        )
